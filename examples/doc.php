...
$qparray = ['Tree Traversal', 'Child Filter', 'Attribute', 'Content Filter', 'Basic Filter',
...
foreach (htmlqp('http://api.jquery.com/api/', 'entry') as $entry) {
	$category = false;
	$category = array_search($entry->find('category:first')->attr('name'), $qparray, true);
	while (null != $entry->next('category')->html()) {
		$category = (array_search($entry->attr('name'), $qparray, true)) ? true : $category;
		if ($category) {
			break;
		}
	}
